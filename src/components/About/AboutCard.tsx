  ...
  ({ className, imageUrl, title, category,  onSave, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "group relative block overflow-hidden bg-card text-card-foreground transition-all duration-300 ease-in-out hover:shadow-lg",
          className,
        )}
        {...props}
      >
        <a  aria-label={title}>
          ...
          <div className="aspect-square overflow-hidden">
            <img
              src={imageUrl}
              alt={title}
              className="h-full w-full border-3 border-black object-cover transition-transform duration-300 ease-in-out group-hover:scale-105"
            />
          </div>
          {/* Card content */}
          <div className="p-4">
