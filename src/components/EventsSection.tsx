		...
		<section className="w-full bg-neutral-100 py-10 md:py-14">
			<div className="mx-auto w-full max-w-300 px-4 md:px-6">
				<header className="mb-3 flex items-center justify-between py-5 md:py-7">
					<h2 className="text-4xl font-bold uppercase tracking-tight text-neutral-900 md:text-5xl">Events</h2>
				...
				>
					<div className="absolute inset-0 flex flex-col justify-start p-6 text-white md:p-10">
						<div className="mb-10 flex items-center gap-5 md:mb-12">
							<span className="inline-flex h-12 w-12 items-center justify-center bg-cyan-400 text-3xl font-bold leading-none text-neutral-950">
								1
							</span>
							<h3 className="text-4xl font-bold uppercase tracking-tight md:text-5xl">{featuredEvent.name}</h3>
						</div>
						...
						<div className="grid max-w-5xl grid-cols-1 gap-6 md:grid-cols-3 md:gap-10">
							{eventHighlights.map((item) => (
								<div key={item.id}>
									...
									<h4 className="mb-2 text-lg font-semibold uppercase tracking-wide">{item.title}</h4>
									<p className="text-xl leading-relaxed text-neutral-100/90">{item.description}</p>
								</div>
							))}
						</div>
					</div>
